...
function mapResult(attribute, keys, options, result) {
    ...
    if (Array.isArray(attribute) && options && options.multiple && !options.raw) {
        ...
        result = _.groupBy(result, _.property(attribute));
    }
...
function rejectOnEmpty(options, result) {
    if (_.isEmpty(result) && options.rejectOnEmpty) {
...
function loaderForBTM(model, joinTableName, foreignKey, foreignKeyField, options = {}) {
    ...
    return new DataLoader(keys => {
        ...
        batch: typeof options.batch !== 'undefined' ? options.batch : true,
        cacheKeyFn: typeof options.cacheKeyFn !== 'undefined' ? options.cacheKeyFn : key => key
    });
}
...
function shimHasMany(target, dtOptions = {}) {
    if (target.get.__wrapped) return;
        ...
        return function bathedGetHasMany(instances, options = {}) {
            ...
            if (!loader) {
                loader = loaderForModel(this.target, this.foreignKey, this.foreignKeyField, {
                    ...loaderOptions,
                    cache: typeof dtOptions.cache !== 'undefined' ? dtOptions.cache : true,
                    batch: typeof dtOptions.batch !== 'undefined' ? dtOptions.batch : true,
                    cacheKeyFn: typeof dtOptions.cacheKeyFn !== 'undefined' ? dtOptions.cacheKeyFn : key => key
                });
...
function shimBelongsToMany(target, dtOptions = {}) {
    if (target.get.__wrapped) return;
...
function createContext(sequelize, options = {}) {
    const loaders = {};
    ...
    shimHasMany(sequelize.constructor.Association.HasMany.prototype, options);
    shimBelongsToMany(sequelize.constructor.Association.BelongsToMany.prototype, options);

    loaders.autogenerated = new LRU({ max: options.max || 500 });
    loaders.resetCache = () => resetCache({ loaders }, sequelize);
...
function resetCache(context, sequelize) {
    ...
    for (const Model of Object.values(sequelize.models)) {
        _.values(Model.associations).forEach(association => {
            if (association.associationType === 'BelongsTo') {
                const Target = association.target;
                if (association.targetKey !== Target.primaryKeyAttribute) {
                    loaders[Target.name].bySingleAttribute[association.targetKey].clearAll();
                }
            } else if (association.associationType === 'HasOne') {
                const Target = association.target;
                loaders[Target.name].bySingleAttribute[association.foreignKey].clearAll();
            }
        });

        Object.keys(loaders[Model.name]).forEach(key => {
            // allow removing custom dataloaders, added outside of dataloader-sequelize
            const possibleDataLoader = loaders[Model.name][key];
            if (possibleDataLoader.constructor.name === 'DataLoader') {
                possibleDataLoader.clearAll();
            }
        });
    }

    loaders.autogenerated.reset();
}
